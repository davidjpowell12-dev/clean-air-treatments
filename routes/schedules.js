...
router.put('/:id', requireAuth, (req, res) => {
  ...
  // Shared logic lives in utils/billing.js so this can't drift from the
  // identical path in routes/applications.js.
  if (status === 'completed' && existing.status !== 'completed' && existing.estimate_id) {
    const { billForCompletedVisit } = require('../utils/billing');
    billForCompletedVisit(db, existing);
  }
...
router.get('/day-mix/:date', requireAuth, (req, res) => {
  ...
  // Pull sqft with fallback: FIRST application's total_area_treated is the
  // primary source — techs measure the property on Round 1, so that's the
  // most reliable number. If no application has been logged yet for this
  // property, fall back to properties.sqft.
